...
1. Raw Bias Detection: Measures direct demographic differences in documentation
2. Adjusted Bias Analysis: Controls for clinical complexity using regression
3. Quality Parity Analysis: Ensures all groups receive complete documentation

The adjusted analysis is critical for distinguishing between legitimate clinical
variation (older patients need more detailed notes) and problematic bias
(systematic under-documentation of certain demographic groups).
"""
...
class MIMICBiasDetector:
    ...
    def _validate_feature_quality(self, df: pd.DataFrame, features: List[str]) -> List[str]:
        ...
        Returns:
            List of features that pass quality checks
        """
        valid_features = []
    ...
    def _perform_statistical_test(self, 
        ...
        Uses t-test for two groups, ANOVA for multiple groups.
        Only performs test if minimum sample size requirements are met.
        ...
        # Get groups with sufficient sample size
        groups = []
        group_names = []
    ...
    def detect_treatment_complexity_bias(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Analyze treatment-related metrics
    ...
    def detect_adjusted_bias(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        This is the critical analysis that determines whether raw bias is legitimate.
        
        Method:
        1. Identifies clinical complexity features (comorbidities, treatments, labs)
        2. Builds regression model: text_length = f(complexity_features + demographics)
        3. Calculates residuals (unexplained variation after accounting for complexity)
        4. Analyzes if residuals vary systematically by demographics
        
        Interpretation:
        - Low residual CV (under 5 percent): Raw variation is explained by legitimate
          clinical differences. No bias mitigation needed.
        - High residual CV (over 5 percent): Variation persists after accounting for
          clinical need, suggesting systematic documentation bias requiring action.
        ...
        # Create working copy with only needed columns to optimize memory
        ...
        # Define clinical complexity features that explain why documentation length varies
        complexity_features = [
        ...
        # Validate features exist and meet quality standards
        ...
        try:
            ...
            # Calculate residuals
            # Residuals represent text length variation not explained by clinical complexity
            predictions = model.predict(X_scaled)
            df_temp['text_chars_residual'] = y.values - predictions
            
            # Analyze residuals by demographics
            residual_stats = {}
            ...
            for demo_col in ['age_group', 'ethnicity_clean', 'gender']:
                ...
                # Determine if variation represents bias
    ...
    def detect_documentation_quality_parity(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        of critical elements like diagnosis, medications, and follow-up instructions.
        
        This analysis checks whether completeness scores and presence of key sections
        are consistent across demographic groups. Disparities here are concerning
        regardless of whether text length variation is legitimate.
        ...
        # Define quality indicators representing critical documentation elements
        quality_metrics = {}
        ...
        # Analyze quality parity for each demographic
        for demo_col in ['age_group', 'ethnicity_clean', 'gender']:
            if demo_col not in df.columns:
                continue
            ...
            for metric, description in quality_metrics.items():
                ...
                # Lower CV indicates better parity
                ...
                # CV under 10 percent indicates good parity
    ...
    def calculate_bias_metrics(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        - Under 5 percent: Minimal variation
        - 5 to 15 percent: Moderate variation
        - Over 15 percent: High variation
    ...
    def interpret_bias_findings(self, bias_report: Dict) -> Dict[str, str]:
        ...
        Translates technical statistical metrics into actionable insights for
        stakeholders. Provides clear verdicts and specific recommendations based
        on the combination of raw, adjusted, and quality parity analyses.
        ...
        if age_cv > 15:
            if adjusted.get('analysis_performed') and age_adjusted_cv is not None:
                if age_adjusted_cv < self.thresholds['residual_cv_threshold']:
                    # High raw CV but low adjusted CV indicates legitimate variation
                    ...
                    # High raw CV and high adjusted CV indicates bias
                    interpretations['age_variation'] = (
                        f"Age Variation Analysis: Raw CV {age_cv:.1f}%, Adjusted CV {age_adjusted_cv:.1f}%\n\n"
    ...
    def create_bias_visualizations(self, df: pd.DataFrame):
        ...
        - Documentation length distributions by demographics
        - Clinical complexity patterns that may explain length differences
        - Quality parity across groups
        ...
        # Plot 1: Documentation length by age with sample sizes
        ...
        # Plot 2: Treatment intensity showing clinical complexity pattern
        ...
        # Plot 3: Abnormal labs showing another complexity indicator
        ...
        # Plot 4: Documentation quality parity check
    ...
    def run_bias_detection_pipeline(self) -> Tuple[Dict, pd.DataFrame]:
        ...
        Pipeline Stages:
        1. Raw Bias Detection - Measures demographic differences without adjustment
        2. Adjusted Bias Analysis - Controls for clinical complexity using regression
        3. Quality Parity Analysis - Ensures all groups receive complete documentation
        4. Interpretation Generation - Translates findings into actionable insights
        5. Visualization Creation - Produces plots for stakeholder review
        ...
        # Stage 1: Raw bias detection across all dimensions
        ...
        # Stage 2: Adjusted bias analysis with clinical complexity controls
        ...
        # Stage 3: Quality parity assessment
        ...
        # Stage 4: Generate interpretations
        ...
        # Print interpretations to console for immediate review
